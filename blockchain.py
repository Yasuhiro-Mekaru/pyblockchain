...
class BlockChain(object):
    ...
    def add_transaction(self, sender_blockchain_address,
                        ...
                        sender_pulic_key=None, signature=None):
        ...
        if self.verify_transaction_signature(sender_pulic_key,
                                             signature,transaction):
            if self.culculate_total_amount(sender_blockchain_address) < float(value):
            ...
            for node in self.neighbours:
                requests.put(
                    ...
                    json = {
                        ...
                        'recipient_blockchain_address': recipient_blockchain_address,
                        'value': value,
                        'sender_public_key': sender_pulic_key,
                        'signature': signature
                    }
                )
    ...
    def verify_transaction_signature(self, sender_pulic_key, signature, transaction):
        sha256 = hashlib.sha256()
        sha256.update(str(transaction).encode('utf-8'))
        message = sha256.digest()
        signature_bytes = bytes().fromhex(signature)
        verifying_key = VerifyingKey.from_string(
            bytes().fromhex(sender_pulic_key), curve=NIST256p)
    ...
    def culculate_total_amount(self, blockchain_address):
        total_amount = 0.0
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if not self.valid_proof(
                    block['transactions'], block['previous_hash'], block['nonce'], MINING_DIFFICULTY):
                return False
